...
def load_tokenize_data(args):
    ...
    if os.path.exists(args.cache_data):
        ...
        datasets = load_dataset("yijunyu/c-to-rust", 'c-to-rust')
        tokenizer = AutoTokenizer.from_pretrained(args.load)

        def preprocess_function(examples):
            # source = [' '.join(ex) for ex in examples["code_tokens"]]
            # target = [' '.join(ex) for ex in examples["docstring_tokens"]]
            source = [' '.join(ex) for ex in examples["before"]]
            target = [' '.join(ex) for ex in examples["after"]]
            ...
            return model_inputs
        print(datasets.column_names["train"])
...
def main(args):
    ...
    model_file = os.path.join(args.save_dir, "final_checkpoint")
    if os.path.exists(model_file):
        model = AutoModelForSeq2SeqLM.from_pretrained(model_file)
        print(f"  ==> Loaded model from {args.save_dir}, model size {model.num_parameters()}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-target-len', default=320, type=int)
    parser.add_argument('--cache-data', default='cache_data/c_to_rust', type=str)
    ...
    # Logging and stuff
    parser.add_argument('--save-dir', default="saved_models/c_to_rust", type=str)
